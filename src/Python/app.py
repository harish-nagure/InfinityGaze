...
def video_processing_traffic_racer():
    ...
    while True:
        ...
        hands, img = detector.findHands(frame, draw=False)
        cv2.putText(frame, "-->", (400, 460), cv2.FONT_HERSHEY_COMPLEX, 1, (255, 255, 255), 1, cv2.LINE_AA)
        ...
        if hands:
            for hand in hands:
                ...
                if lmList['type'] == 'Left' and sum(fingerUp) >= 2:
                    print(lmList['type'])
                    cv2.putText(frame, "-->", (400, 460), cv2.FONT_HERSHEY_COMPLEX, 1, (255, 0, 255), 5, cv2.LINE_AA)
                    pyautogui.keyDown('right')
                    # pyautogui.keyUp('left')
                    left_hand_pressed = True
                elif lmList['type'] == 'Right' and sum(fingerUp) >= 2:
